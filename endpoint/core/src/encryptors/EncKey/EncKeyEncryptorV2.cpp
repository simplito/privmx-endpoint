...
DecryptedEncKeyV2 EncKeyEncryptorV2::decrypt(
    ...
    try {
        ...
        if (expandedDio.fieldChecksums.at("encryptedKey") !=
            privmx::crypto::Crypto::sha256(encryptedEncKey.encryptedKey)) {
...
void EncKeyEncryptorV2::assertDataFormat(const server::EncryptedKeyEntryDataV2& encryptedEncKey) {
    if (encryptedEncKey.version != EncryptionKeyDataSchema::Version::VERSION_2 ||
