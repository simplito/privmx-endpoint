...
int privmx_endpoint_stream_extractKey(
    const privmx_endpoint_stream_Key keys[],
    size_t index,
    const char** keyId,
...
int privmx_endpoint_stream_newProxyWebRTC(
    ...
    *outPtr = (privmx_endpoint_stream_ProxyWebRTC*
    )new std::shared_ptr<stream::ProxyWebRTC>(std::make_shared<stream::ProxyWebRTC>(webRTCInterface));
    return 1;
}
