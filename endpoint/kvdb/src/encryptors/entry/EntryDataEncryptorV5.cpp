...
DecryptedKvdbEntryDataV5 EntryDataEncryptorV5::decrypt(
    ...
    try {
        ...
        result.privateMeta = _dataEncryptor.decodeAndDecryptAndVerify(
            encryptedEntryData.privateMeta, authorPublicKey, encryptionKey
        );
        ...
        result.internalMeta = encryptedEntryData.internalMeta.has_value() ?
            std::make_optional(_dataEncryptor.decodeAndDecryptAndVerify(
                encryptedEntryData.internalMeta.value(), authorPublicKey, encryptionKey
            )) :
            std::nullopt;
...
core::DataIntegrityObject EntryDataEncryptorV5::getDIOAndAssertIntegrity(
    const server::EncryptedKvdbEntryDataV5& encryptedEntryData
) {
    assertDataFormat(encryptedEntryData);
    auto dio = _DIOEncryptor.decodeAndVerify(encryptedEntryData.dio);
    if (dio.structureVersion != KvdbEntryDataSchema::Version::VERSION_5 ||
        ...
        (encryptedEntryData.internalMeta.has_value() &&
         dio.fieldChecksums.at("internalMeta") !=
             privmx::crypto::Crypto::sha256(encryptedEntryData.internalMeta.value()))) {
...
void EntryDataEncryptorV5::assertDataFormat(const server::EncryptedKvdbEntryDataV5& encryptedEntryData) {
    if (encryptedEntryData.version != KvdbEntryDataSchema::Version::VERSION_5 ||
        ...
        throw InvalidEncryptedKvdbEntryDataVersionException(
            std::to_string(encryptedEntryData.version) +
            " expected version: " +
            std::to_string(KvdbEntryDataSchema::Version::VERSION_5)
        );
    }
}
