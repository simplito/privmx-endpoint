...
struct VideoData : public Data {
    VideoData(
        ...
        Data(_type, _streamId, _track), 
        w(_w), h(_h), frameData(_frameData)
    {}
    const int64_t w; 
    const int64_t h; 
...
public:
    virtual void OnRemoteTrack(Track tack, TrackAction action);
    virtual void OnData(std::shared_ptr<Data> data);
protected:
