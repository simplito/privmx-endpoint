...
/**
 * Unit tests for turning plans into the complete tree state the bridge receives, with real EC keys.
 *
 * The merge these tests cover is easy to get subtly wrong: a removal has to replace exactly the nodes it
 * refreshed and drop exactly the edges that refresh invalidated, while growth can re-parent a leaf and so
 * invalidate an edge that was correct a moment earlier. A mistake here does not produce a wrong key — it
 * produces a state the server rejects, or worse, one it accepts while a member can no longer climb.
 *
 * So the tests check both halves: the shape of the state, and that the members who should still be able to climb
 * it actually can. The server's own view of the same states is checked from the bridge's test suite, against
...
TEST(TreeWireBuild, SeatsEveryMemberAndPublishesEveryInternalNode) {
...
TEST(TreeWireBuild, EveryMemberCanClimbTheStateThatWasSubmitted) {
...
TEST(TreeWireBuild, TheGrantEdgeCarriesTheEpochRatherThanANodeGeneration) {
...
TEST(TreeWireBuild, AOneMemberGroupWrapsTheGrantKeyStraightToTheMember) {
...
TEST(TreeWireRemoval, BlanksTheLeafAndKeepsTheTreeTheSameSize) {
...
TEST(TreeWireRemoval, ReplacesEveryRefreshedNodeExactlyOnce) {
    auto fixture = build(8, 5);
    const RemovalOutcome outcome = removeMember(*fixture, 3, 0);
...
TEST(TreeWireRemoval, SECURITY_LeavesNoEdgeAddressedToTheDepartingMember) {
    auto fixture = build(8, 5);
...
TEST(TreeWireRemoval, SECURITY_TheRemovedMemberCanNoLongerClimb) {
...
TEST(TreeWireRemoval, EveryRemainingMemberCanStillClimb) {
...
TEST(TreeWireRemoval, TheGrantEdgeMovesToTheNewEpoch) {
...
TEST(TreeWireRemoval, EveryEdgeNamesTheCurrentGenerationOfBothEndpoints) {
...
TEST(TreeWireAddition, SECURITY_DoesNotMoveTheEpoch) {
    // If an addition rotated the grant key, every container the group can read would go stale — one new member
    // would force the whole group to re-key, which is the cost the grant indirection exists to avoid.
    auto fixture = build(4, 5);
...
TEST(TreeWireAddition, LeavesEveryExistingNodeKeyUntouched) {
    auto fixture = build(4, 5);
    const AdditionOutcome outcome = addMember(*fixture, publicOf(fixture->members));
...
TEST(TreeWireAddition, TheNewcomerAndEveryoneElseCanClimb) {
...
TEST(TreeWireAddition, GrowthThatReParentsALeafDropsTheSupersededEdge) {
    // N=3 -> 4 moves leaf 2 under a node that did not exist. The edge naming its old parent describes a topology
    // that is gone, and a state carrying both would be rejected — and rightly, since only one of them is real.
...
TEST(TreeWireAddition, GrowthMintsANewRootAndRelinksTheGrantEdgeToIt) {
...
TEST(TreeWireAddition, SeatingIntoABlankCostsOneWrapAndAddsNoNode) {
    auto fixture = build(4, 5);
