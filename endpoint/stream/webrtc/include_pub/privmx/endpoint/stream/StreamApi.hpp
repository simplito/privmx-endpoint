...
public:
     ...
     * @param streamRoomTtl grace period (ms) the room stays open after the last participant leaves before being
    ...
    std::string createStreamRoom(
        ...
        const std::optional<int64_t>& streamRoomTtl = std::nullopt
    );
    ...
    /**
     * Unsubscribes from remote streams for the given subscription handle.
