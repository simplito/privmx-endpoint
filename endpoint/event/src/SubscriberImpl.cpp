...
const std::set<std::string> SubscriberImpl::_forbiddenChannelsNames{INTERNAL_EVENT_CHANNEL_NAME};
...
std::string SubscriberImpl::buildQuery(
    ...
    if (it == _forbiddenChannelsNames.end() || enableAllChannelNames) {
        return core::SubscriptionQueryObj(getChannelPath(channelName), getSelectors(selectorType, selectorId))
    ...
    throw ForbiddenChannelNameException(
        ("Forbidden channelName. Forbidden channel names are: " +
         forbiddenChannelsNamesString +
         ". Received \"" +
         channelName +
         "\"")
    );
