...
std::string BackendRequester::backendRequest(
    ...
    if (mode == 0) {
        ...
        return httpChannel
            ->send(
                requestString, "/api", headers, privmx::utils::CancellationToken::create(), contentType, false, false
            )
            .get();
    } else {
...
std::string BackendRequester::_backendRequest(
    ...
    auto requestString{utils::Utils::stringify(request_json)};
    return httpChannel
        ->send(requestString, "/api", headers, privmx::utils::CancellationToken::create(), contentType, false, false)
        .get();
}
