...
TEST(PrivateKeyTest, KeyDerivation2BasicTests) {
    ...
    EXPECT_EQ(priv1.deriveB(publ2),Utils::s2b(expected_derPriv1Pub2));
    EXPECT_EQ(priv2.deriveB(publ1),Utils::s2b(expected_derPriv2Pub1));
    EXPECT_EQ(priv3.deriveB(publ4),Utils::s2b(expected_derPriv3Pub4));
    EXPECT_EQ(priv4.deriveB(publ3),Utils::s2b(expected_derPriv4Pub3));
}
