...
AudioTrackSinkImpl::AudioTrackSinkImpl(
    ...
    : _roomOnTrackInterface(roomOnTrackInterface), _streamOnTrackInterfacesMap(streamOnTrackInterfacesMap),
      ...
      _track(track){LOG_TRACE("AudioTrackSinkImpl created")} AudioTrackSinkImpl::AudioTrackSinkImpl(
          const std::vector<std::string>& streamIds,
          libwebrtc::scoped_refptr<libwebrtc::RTCMediaTrack> track
      )
...
void AudioTrackSinkImpl::updateStreamOnTrackInterfacesMap(
    std::shared_ptr<privmx::utils::ThreadSaveMap<std::string, std::shared_ptr<OnTrackInterface>>>
        streamOnTrackInterfacesMap
) {
    std::unique_lock<std::mutex> lock(m);
...
void AudioTrackSinkImpl::OnData(
    ...
    std::unique_lock<std::mutex> lock(m);
    std::shared_ptr<AudioData> audioData = std::make_unique<AudioData>(
        _streamIds, _track->id().std_string(), audio_data, bits_per_sample, sample_rate, number_of_channels,
        number_of_frames
    );
