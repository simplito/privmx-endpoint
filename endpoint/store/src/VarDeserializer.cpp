...
template<>
store::EventType VarDeserializer::deserialize<store::EventType>(
    ...
    throw InvalidParamsException(
        name + " | " + ("Unknown store::EventType value, received " + std::to_string(val.convert<int64_t>()))
    );
}
...
store::EventSelectorType VarDeserializer::deserialize<store::EventSelectorType>(
    ...
    throw InvalidParamsException(
        name + " | " + ("Unknown store::EventSelectorType value, received " + std::to_string(val.convert<int64_t>()))
    );
}