...
core::DataIntegrityObject FileMetaEncryptorV5::getDIOAndAssertIntegrity(
    const server::EncryptedFileMetaV5& encryptedFileMeta
) {
    assertDataFormat(encryptedFileMeta);
    auto dio = _DIOEncryptor.decodeAndVerify(encryptedFileMeta.dio);
    if (dio.structureVersion != FileDataSchema::Version::VERSION_5 ||
        ...
        dio.fieldChecksums.at("internalMeta") !=
            privmx::crypto::Crypto::sha256(encryptedFileMeta.internalMeta.value())) {
...
void FileMetaEncryptorV5::assertDataFormat(const server::EncryptedFileMetaV5& encryptedFileMeta) {
    if (encryptedFileMeta.version != FileDataSchema::Version::VERSION_5 ||
        ...
        throw InvalidEncryptedStoreFileMetaVersionException(
            std::to_string(encryptedFileMeta.version) +
            " expected version: " +
            std::to_string(FileDataSchema::Version::VERSION_5)
        );
    }
}
