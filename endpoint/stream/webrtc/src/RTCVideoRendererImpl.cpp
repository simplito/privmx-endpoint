...
int FrameImpl::ConvertToRGBA(uint8_t* dst_argb, int dst_stride_argb, int dest_width, int dest_height) {
    return _frame->ConvertToARGB(
        libwebrtc::RTCVideoFrame::Type::kRGBA, dst_argb, dst_stride_argb, dest_width, dest_height
    );
}
...
RTCVideoRendererImpl::RTCVideoRendererImpl(
    ...
    std::shared_ptr<privmx::utils::ThreadSaveMap<std::string, std::shared_ptr<OnTrackInterface>>>
        streamOnTrackInterfacesMap,
    const std::vector<std::string>& streamIds,
    libwebrtc::scoped_refptr<libwebrtc::RTCMediaTrack> track
)
    : _roomOnTrackInterface(roomOnTrackInterface), _streamOnTrackInterfacesMap(streamOnTrackInterfacesMap),
      _streamIds(streamIds),
      _track(track){LOG_TRACE("RTCVideoRendererImpl created")} RTCVideoRendererImpl::RTCVideoRendererImpl(
          const std::vector<std::string>& streamIds,
          libwebrtc::scoped_refptr<libwebrtc::RTCMediaTrack> track
      )
    : _roomOnTrackInterface(nullptr), _streamOnTrackInterfacesMap(nullptr), _streamIds(streamIds),
      _track(track){LOG_TRACE("RTCVideoRendererImpl created")} RTCVideoRendererImpl::~RTCVideoRendererImpl() {
...
void RTCVideoRendererImpl::updateStreamOnTrackInterfacesMap(
    std::shared_ptr<privmx::utils::ThreadSaveMap<std::string, std::shared_ptr<OnTrackInterface>>>
        streamOnTrackInterfacesMap
) {
    std::unique_lock<std::mutex> lock(m);
...
void RTCVideoRendererImpl::OnFrame(libwebrtc::scoped_refptr<libwebrtc::RTCVideoFrame> frame) {
    std::unique_lock<std::mutex> lock(m);
    std::shared_ptr<VideoData> videoData = std::make_unique<VideoData>(
        _streamIds, _track->id().std_string(), frame->width(), frame->height(), std::make_shared<FrameImpl>(frame)
    );
