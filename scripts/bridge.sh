...
Usage: $0 [options]
  ...
  --dataset NAME|DIR     Dataset to seed the Bridge with: a directory, or a name under
                         test/env/datasets/ (default: $DATASET)
...
# A dataset is either a path or a name under test/env/datasets/
DATASET_DIR="$DATASET"
[[ -d "$DATASET_DIR" ]] || DATASET_DIR="test/env/datasets/$DATASET"
...
if [[ "$(mongo_health)" != "healthy" ]]; then
    ...
    (cd test/env && docker compose up -d) || exit 1
    STARTED_BACKEND=1
...
python3 test/runner/e2e_runner.py --start-bridge --dataset-dir "$DATASET_DIR" \
