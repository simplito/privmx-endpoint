...
public:
    ...
    virtual std::tuple<Pson::BinaryString, T, Pson::BinaryString> decryptAndGetSign(
...
public:
    std::string encrypt(const T& data, const std::string& key) {
        ...
        return utils::Base64::from(
            privmx::crypto::CryptoPrivmx::privmxEncrypt(
                privmx::crypto::CryptoPrivmx::privmxOptAesWithSignature(), buffer, key
            )
        );
    }
    ...
    std::tuple<Pson::BinaryString, T, Pson::BinaryString> decryptAndGetSign(
        const std::string& data,
        const std::string& key
    ) {
...
public:
    ...
    std::tuple<Pson::BinaryString, Pson::BinaryString, Pson::BinaryString> decryptAndGetSign(
        const std::string& data,
        const std::string& key
    ) {
        Pson::BinaryString plain = privmx::crypto::CryptoPrivmx::privmxDecrypt(
