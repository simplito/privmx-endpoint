...
void StructValidator<VerificationOptions>::validate(const VerificationOptions& value, const std::string& stack_trace) {
    if(value.instanceId.has_value() && !value.publicKey.has_value()) {
        throw InvalidParamsException(stack_trace + " | " + ("Invalid VerificationOptions, publicKey cannot be Null, when instanceId is not Null"));
    }
}
