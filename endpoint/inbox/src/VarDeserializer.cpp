...
inbox::FilesConfig VarDeserializer::deserialize<inbox::FilesConfig>(
    ...
    return {
        .minCount = deserialize<int64_t>(obj->get("minCount"), name + ".minCount"),
...
template<>
inbox::EventType VarDeserializer::deserialize<inbox::EventType>(
    const Poco::Dynamic::Var& val,
    const std::string& name
) {
    ...
    throw InvalidParamsException(
        name + " | " + ("Unknown inbox::EventType value, received " + std::to_string(val.convert<int64_t>()))
    );
}
...
inbox::EventSelectorType VarDeserializer::deserialize<inbox::EventSelectorType>(
    ...
    throw InvalidParamsException(
        name + " | " + ("Unknown inbox::EventSelectorType value, received " + std::to_string(val.convert<int64_t>()))
    );
}