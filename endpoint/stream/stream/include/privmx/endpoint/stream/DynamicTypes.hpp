...
#define VERSIONED_DATA_FIELDS(F) F(version, int64_t)
...
#define STREAM_CUSTOM_EVENT_DATA_FIELDS(F) F(streamRoomId, std::string)
JSON_STRUCT(StreamCustomEventData, STREAM_CUSTOM_EVENT_DATA_FIELDS);

#define STREAM_KEY_MANAGEMENT_EVENT_FIELDS(F) F(subtype, std::string)
...
#define REQUEST_KEY_RESPOND_EVENT_FIELDS(F) F(encKey, StreamEncKey)
JSON_STRUCT_EXT(RequestKeyRespondEvent, StreamKeyManagementEvent, REQUEST_KEY_RESPOND_EVENT_FIELDS);

#define UPDATE_KEY_EVENT_FIELDS(F) F(encKey, NewStreamEncKey)
JSON_STRUCT_EXT(UpdateKeyEvent, StreamKeyManagementEvent, UPDATE_KEY_EVENT_FIELDS);

#define UPDATE_KEY_ACK_EVENT_FIELDS(F) F(keyId, std::string)
