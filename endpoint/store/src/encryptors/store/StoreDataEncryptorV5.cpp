...
server::EncryptedStoreDataV5 StoreDataEncryptorV5::encrypt(const StoreDataToEncryptV5& storeData,
    ...
    result.version(StoreDataStructVersion::VERSION_5);
    ...
    core::ExpandedDataIntegrityObject expandedDio = {storeData.dio, .structureVersion=StoreDataStructVersion::VERSION_5, .fieldChecksums=fieldChecksums};
...
DecryptedStoreDataV5 StoreDataEncryptorV5::decrypt(
    ...
    result.statusCode = 0;
    result.dataStructureVersion = StoreDataStructVersion::VERSION_5;
    try {
...
core::DataIntegrityObject StoreDataEncryptorV5::getDIOAndAssertIntegrity(const server::EncryptedStoreDataV5& encryptedStoreData) {
    ...
    if (
        dio.structureVersion != StoreDataStructVersion::VERSION_5 ||
...
void StoreDataEncryptorV5::assertDataFormat(const server::EncryptedStoreDataV5& encryptedStoreData) {
    if (encryptedStoreData.versionEmpty() ||
        encryptedStoreData.version() != StoreDataStructVersion::VERSION_5 ||
        encryptedStoreData.publicMetaEmpty() ||
        encryptedStoreData.privateMetaEmpty() ||
        encryptedStoreData.authorPubKeyEmpty()
    ) {
        throw InvalidEncryptedStoreDataVersionException(std::to_string(encryptedStoreData.version()) + " expected version: " + std::to_string(StoreDataStructVersion::VERSION_5));
    }
}