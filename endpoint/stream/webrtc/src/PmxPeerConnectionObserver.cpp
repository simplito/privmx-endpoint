...
PmxPeerConnectionObserver::PmxPeerConnectionObserver(
    ...
    : _peerConnectionFactory(peerConnectionFactory), _streamRoomId(streamRoomId), _currentKeys(keys), _options(options),
      _apiLow(apiLow), _roomOnTrackInterface(nullptr),
      _streamOnTrackInterfacesMap(
          std::make_shared<privmx::utils::ThreadSaveMap<std::string, std::shared_ptr<OnTrackInterface>>>()
      ) {}
...
void PmxPeerConnectionObserver::OnIceCandidate(
    [[maybe_unused]] libwebrtc::scoped_refptr<libwebrtc::RTCIceCandidate> candidate
) {
    LOG_DEBUG("STREAMS ", "API ", _streamRoomId + ": ON ICE CANDIDATE")
    if (_onIceCandidate.has_value())
        _onIceCandidate.value()(candidate);
}
...
void PmxPeerConnectionObserver::OnRemoveStream(
    [[maybe_unused]] libwebrtc::scoped_refptr<libwebrtc::RTCMediaStream> stream
) {
...
void PmxPeerConnectionObserver::OnDataChannel(libwebrtc::scoped_refptr<libwebrtc::RTCDataChannel> data_channel) {
    ...
    std::shared_ptr<DataChannelImpl> dataChannelImpl = std::make_shared<DataChannelImpl>(
        roomOnTrackInterface, _apiLow, data_channel, _streamRoomId
    );
...
void PmxPeerConnectionObserver::OnTrack(
    [[maybe_unused]] libwebrtc::scoped_refptr<libwebrtc::RTCRtpTransceiver> transceiver
) {
    LOG_DEBUG("STREAMS ", "API ", _streamRoomId + ": ON TRACK")
    LOG_TRACE("STREAMS ", "API ", _streamRoomId + ": ON TRACK done")
}
void PmxPeerConnectionObserver::OnAddTrack(
    [[maybe_unused]] libwebrtc::vector<libwebrtc::scoped_refptr<libwebrtc::RTCMediaStream>> streams,
    [[maybe_unused]] libwebrtc::scoped_refptr<libwebrtc::RTCRtpReceiver> receiver
) {
    ...
    if (dataType == DataType::VIDEO) {
        ...
        std::shared_ptr<RTCVideoRendererImpl> renderer = std::make_shared<RTCVideoRendererImpl>(
        ...
        std::shared_ptr<AudioTrackSinkImpl> sink = std::make_shared<AudioTrackSinkImpl>(
            roomOnTrackInterface, _streamOnTrackInterfacesMap, streamIds, track
        );
...
void PmxPeerConnectionObserver::OnRemoveTrack(
    [[maybe_unused]] libwebrtc::scoped_refptr<libwebrtc::RTCRtpReceiver> receiver
) {
    ...
    if (dataType == DataType::VIDEO)
        _RTCVideoRenderers.erase(receiver->track()->id().std_string());
}
...
void PmxPeerConnectionObserver::UpdateCurrentKeys(std::shared_ptr<privmx::webrtc::KeyStore> newKeys) {
    ...
    LOG_DEBUG(
        "STREAMS", "PmxPeerConnectionObserver",
        "PmxPeerConnectionObserver::UpdateCurrentKeys _frameCryptors.size()=" + std::to_string(_frameCryptors.size())
    );
    _frameCryptors.forAll([&]([[maybe_unused]] const std::string& id,
                              const std::shared_ptr<privmx::webrtc::FrameCryptor>& frameCryptor) {
        LOG_DEBUG(
            "STREAMS", "PmxPeerConnectionObserver", "PmxPeerConnectionObserver::UpdateCurrentKeys::forAll::single"
        );
...
void PmxPeerConnectionObserver::setOnTrackInterface(std::shared_ptr<OnTrackInterface> roomOnTrackInterface) {
    ...
    _RTCVideoRenderers.forAll([roomOnTrackInterface](
                                  [[maybe_unused]] const std::string& streamId,
                                  const std::shared_ptr<privmx::endpoint::stream::RTCVideoRendererImpl>& renderer
                              ) { renderer->updateRoomOnTrackInterface(roomOnTrackInterface); });
    _audioTrackSinks.forAll([roomOnTrackInterface](
                                [[maybe_unused]] const std::string& streamId,
                                const std::shared_ptr<privmx::endpoint::stream::AudioTrackSinkImpl>& renderer
                            ) { renderer->updateRoomOnTrackInterface(roomOnTrackInterface); });
    _dataChannels.forAll([roomOnTrackInterface](
                             [[maybe_unused]] const std::string& trackId,
                             const std::shared_ptr<DataChannelImpl>& renderer
                         ) { renderer->updateOnTrackInterface(roomOnTrackInterface); });
}
void PmxPeerConnectionObserver::addOnTrackInterfaceForSingleStream(
