...
#include "privmx/endpoint/crypto/cinterface/crypto.h"
...
int privmx_endpoint_newCryptoApi(CryptoApi** outPtr) {
    crypto::CryptoApiVarInterface* ptr = new crypto::CryptoApiVarInterface(
...
int privmx_endpoint_newExtKey(ExtKey** outPtr) {
    crypto::ExtKeyVarInterface* ptr = new crypto::ExtKeyVarInterface(
        core::VarSerializer::Options{.addType = true, .binaryFormat = core::VarSerializer::Options::PSON_BINARYSTRING}
    );
    *outPtr = (ExtKey*)ptr;
    return 1;
