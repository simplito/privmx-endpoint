...
void Validator::validateMaxLength(const string& value, size_t max_len, const string& stack_trace) {
    if (value.length() > max_len) {
        throw InvalidParamsException(
            stack_trace +
            " | " +
            ("Invalid max length, expected max length " +
             to_string(max_len) +
             ", received length " +
...
void Validator::validateLength(const string& value, size_t len, const string& stack_trace) {
    if (value.length() != len) {
        throw InvalidParamsException(
            stack_trace +
            " | " +
            ("Invalid length, expected length " + to_string(len) + ", received length " + to_string(value.length()))
        );
    }
}

void Validator::validateLengthSize(
...
void Validator::validateNumberPositive(int64_t value, const std::string& stack_trace) {
    if (value <= 0) {
        throw InvalidParamsException(
            stack_trace + " | " + ("Invalid number, expected positive number, received " + to_string(value))
        );
    }
}

void Validator::validateNumberPositive(
    [[maybe_unused]] uint64_t value,
    [[maybe_unused]] const std::string& stack_trace
) {
    if (value <= 0) {
        throw InvalidParamsException(
            stack_trace + " | " + ("Invalid number, expected positive number, received " + to_string(value))
...
void Validator::validateNumberNegative(int64_t value, const std::string& stack_trace) {
    if (value >= 0) {
        throw InvalidParamsException(
            stack_trace + " | " + ("Invalid number, expected negative number, received " + to_string(value))
        );
    }
}
...
void Validator::validateNumberNonNegative(
    [[maybe_unused]] uint64_t value,
    [[maybe_unused]] const std::string& stack_trace
) {}
...
void Validator::validateNumberNonNegative(int32_t value, const std::string& stack_trace) {
    if (value < 0) {
        throw InvalidParamsException(
            stack_trace + " | " + ("Invalid number, expected no negative number, received " + to_string(value))
        );
    }
}
...
void Validator::validateNumberNonPositive(int64_t value, const std::string& stack_trace) {
    if (value > 0) {
        throw InvalidParamsException(
            stack_trace + " | " + ("Invalid number, expected no positive number, received " + to_string(value))
        );
    }
}
...
void Validator::validateEnumParamString(
    const std::string& value,
    const std::vector<std::string>& allowed_values,
    const std::string& param_string_name,
...
void Validator::validateEventType(const Event& value, const std::string& type, const std::string& stack_trace) {
    if (value.type != type) {
        throw InvalidParamsException(
            stack_trace + " | " + ("Invalid Event type, expected '" + type + "', received '" + value.type + "'")
        );
    }
}

void Validator::validatePagingQuery(
    const PagingQuery& value,
    const std::vector<std::string>& sort_by_field,
    const std::string& stack_trace
) {
    Validator::validateNumberPositive(value.limit, stack_trace + "PagingQuery.limit");
    Validator::validateNumberNonNegative(value.skip, stack_trace + "PagingQuery.skip");
    ...
    if (value.sortBy.has_value()) {
        Validator::validateEnumParamString(
            value.sortBy.value(), sort_by_field, "sortBy", stack_trace + "PagingQuery.sortBy"
        );
    }
...
void Validator::validateBufferSize(
    ...
    if (value.size() < min_len || value.size() > max_len) {
        throw InvalidParamsException(
            stack_trace +
            " | " +
            ("Invalid BufferSize, expected size " +
             to_string(min_len) +
             "-" +
             to_string(max_len) +
             ", received size " +
...
void Validator::validateJSON(const std::string& value, const std::string& stack_trace) {
    ...
    } catch (...) { throw InvalidParamsException(stack_trace + " | " + ("Invalid JSON")); }
}
...
void StructValidator<LibPlatformDisconnectedEvent>::validate(
    const LibPlatformDisconnectedEvent& value,
    const std::string& stack_trace
) {
    Validator::validateEventType(value, "libPlatformDisconnected", stack_trace + ".type");
}

void StructValidator<PKIVerificationOptions>::validate(
    const PKIVerificationOptions& value,
    const std::string& stack_trace
) {
    if (value.bridgeInstanceId.has_value() && !value.bridgePubKey.has_value()) {
        throw InvalidParamsException(
            stack_trace +
            " | " +
            ("Invalid PKIVerificationOptions, bridgePubKey cannot be Null, when bridgeInstanceId is not Null")
        );
    }
}
