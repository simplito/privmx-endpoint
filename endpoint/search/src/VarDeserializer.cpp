...
search::IndexMode VarDeserializer::deserialize<search::IndexMode>(
    ...
    throw InvalidParamsException(
        name + " | " + ("Unknown search::IndexMode value, received " + std::to_string(val.convert<int64_t>()))
    );
}

template<>
search::Document VarDeserializer::deserialize<search::Document>(
    const Poco::Dynamic::Var& val,
    const std::string& name
) {
    Poco::JSON::Object::Ptr obj = val.extract<Poco::JSON::Object::Ptr>();
    return {
        .documentId = deserialize<int64_t>(obj->get("documentId"), name + ".documentId"),
