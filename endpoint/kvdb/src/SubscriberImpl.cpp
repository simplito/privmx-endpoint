...
const std::map<EventType, std::string> SubscriberImpl::_eventTypeNames = {
    {EventType::KVDB_CREATE, "create"},  {EventType::KVDB_UPDATE, "update"},
    {EventType::KVDB_DELETE, "delete"},  {EventType::KVDB_STATS, "stats"},
    {EventType::ENTRY_CREATE, "create"}, {EventType::ENTRY_UPDATE, "update"},
    {EventType::ENTRY_DELETE, "delete"}, {EventType::COLLECTION_CHANGE, "collectionChanged"}
};
const std::map<EventType, std::set<SubscriberImpl::EventInternalSelectorType>>
    SubscriberImpl::_eventTypeAllowedSelectorTypes = {
        ...
        {EventType::ENTRY_UPDATE,
         {EventInternalSelectorType::CONTEXT_ID, EventInternalSelectorType::KVDB_ID,
          EventInternalSelectorType::ENTRY_ID}},
...
const std::map<EventType, std::string> SubscriberImpl::_readableEventType = {
    {EventType::KVDB_CREATE, "KVDB_CREATE"},   {EventType::KVDB_UPDATE, "KVDB_UPDATE"},
    {EventType::KVDB_DELETE, "KVDB_DELETE"},   {EventType::KVDB_STATS, "KVDB_STATS"},
    {EventType::ENTRY_CREATE, "ENTRY_CREATE"}, {EventType::ENTRY_UPDATE, "ENTRY_UPDATE"},
...
std::vector<core::SubscriptionQueryObj::QuerySelector> SubscriberImpl::getSelectors(
    EventInternalSelectorType selectorType,
    const std::string& selectorId,
    const std::optional<std::string>& extraSelectorData
) {
    if (selectorType == EventInternalSelectorType::ENTRY_ID && extraSelectorData.has_value()) {
        return {core::SubscriptionQueryObj::QuerySelector{
            ...
            .selectorValue = extraSelectorData.value() + ":" + selectorId
        }};
    } else if (selectorType == EventInternalSelectorType::ENTRY_ID && !extraSelectorData.has_value()) {
        throw core::InvalidParamsException(("EventInternalSelectorType::ENTRY_ID missing extraSelectorData"));
    }
    return {core::SubscriptionQueryObj::QuerySelector{
...
std::string SubscriberImpl::buildQuery(
    ...
    SubscriberImpl::EventInternalSelectorType
    ...
    if (it != allowedSelectorTypes.end()) {
        ...
        return core::SubscriptionQueryObj(getChannelPath(eventType), getSelectors(selectorTypeInternal, selectorId))
    ...
    throw core::InvalidParamsException(
        ("Invalid SelectorType for EventType::" +
         _readableEventType.at(eventType) +
         ", expected " +
         allowedSelectorTypesString +
         ", received " +
         _readableSelectorType.at(selectorTypeInternal))
    );
}

std::string SubscriberImpl::buildQueryForSelectedEntry(
    EventType eventType,
    const std::string& kvdbId,
    const std::string& kvdbEntryId
) {
    std::set<EventInternalSelectorType> allowedSelectorTypes = _eventTypeAllowedSelectorTypes.at(eventType);
    std::set<EventInternalSelectorType>::iterator it = allowedSelectorTypes.find(EventInternalSelectorType::ENTRY_ID);
    if (it != allowedSelectorTypes.end()) {

        return core::SubscriptionQueryObj(
                   getChannelPath(eventType), getSelectors(EventInternalSelectorType::ENTRY_ID, kvdbEntryId, kvdbId)
        )
            .toSubscriptionQueryString();
    }
    std::string allowedSelectorTypesString;
    ...
    throw core::InvalidParamsException(
        ("Invalid SelectorType for EventType::" +
         ...
         allowedSelectorTypesString +
         ", received " +
         _readableSelectorType.at(EventInternalSelectorType::ENTRY_ID))
    );
}
