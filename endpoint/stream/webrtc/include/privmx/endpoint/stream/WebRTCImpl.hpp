...
public:
    ...
    void updatePeerConnectionWithLocalStream(
        ...
        const std::vector<std::pair<std::string, libwebrtc::scoped_refptr<libwebrtc::RTCAudioTrack>>>&
            audioTracksToRemove,
        const std::vector<std::pair<std::string, libwebrtc::scoped_refptr<libwebrtc::RTCVideoTrack>>>&
            videoTracksToRemove,
        const std::optional<std::pair<std::string, std::function<void(std::string)>*>>& dataChannel
    );
...
private:
    ...
    static std::shared_ptr<privmx::webrtc::KeyStore> createWebRtcKeyStore(
