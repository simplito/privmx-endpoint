...
std::string ThreadApiImpl::_createThreadEx(
    ...
    create_thread_model.data = _threadDataEncryptorV5.encrypt(threadDataToEncrypt, _userPrivKey, threadKey.key).toJSON();
...
void ThreadApiImpl::updateThread(
    ...
    auto currentThreadResourceId = currentThread.resourceId.empty() ? core::EndpointUtils::generateId() : currentThread.resourceId;
    ...
    auto usersKeysResolver {core::UsersKeysResolver::create(currentThread.users, currentThread.managers, users, managers, forceGenerateNewKey, currentThreadKey)};
...
void ThreadApiImpl::updateMessage(
    ...
    try {
        ...
        return updateMessageRequest(messageId, message.resourceId.empty() ? core::EndpointUtils::generateId() : message.resourceId, message.threadId, publicMeta, privateMeta, data, currentKeys);
    } catch (const privmx::utils::PrivmxException& e) {
        if (core::ExceptionConverter::convert(e).getCode() == privmx::endpoint::server::InvalidThreadKeyException().getCode()) {
...
void ThreadApiImpl::processNotificationEvent(const std::string& type, const core::NotificationEvent& notification) {
    ...
    _guardedExecutor->exec([&, type, notification]() {
        if (type == "threadCreated") {
            auto raw = server::ThreadInfo_c_struct::formJSON(notification.data);
            if((raw.type.empty() ? std::string(THREAD_TYPE_FILTER_FLAG) : raw.type) == THREAD_TYPE_FILTER_FLAG) {
            ...
            auto raw = server::ThreadDeletedEventData_c_struct::formJSON(notification.data);
            if((raw.type.empty() ? std::string(THREAD_TYPE_FILTER_FLAG) : raw.type) == THREAD_TYPE_FILTER_FLAG) {
            ...
            auto raw = server::ThreadStatsEventData_c_struct::formJSON(notification.data);
            if((raw.type.empty() ? std::string(THREAD_TYPE_FILTER_FLAG) : raw.type) == THREAD_TYPE_FILTER_FLAG) {
            ...
            auto raw = server::ThreadMessageEventData_c_struct::formJSON(notification.data);
            if((raw.containerType.empty() ? std::string(THREAD_TYPE_FILTER_FLAG) : raw.containerType) == THREAD_TYPE_FILTER_FLAG) {
            ...
            auto raw = server::ThreadDeletedMessageEventData_c_struct::formJSON(notification.data);
            if((raw.containerType.empty() ? std::string(THREAD_TYPE_FILTER_FLAG) : raw.containerType) == THREAD_TYPE_FILTER_FLAG) {
            ...
            auto raw = core::server::CollectionChangedEventData_c_struct::formJSON(notification.data);
            if((raw.containerType.empty() ? std::string(THREAD_TYPE_FILTER_FLAG) : raw.containerType) == THREAD_TYPE_FILTER_FLAG) {
...
ThreadDataSchema::Version ThreadApiImpl::getThreadEntryDataStructureVersion(server::Thread2DataEntry_c_struct threadEntry) {
    if (threadEntry.data.type() == typeid(Poco::JSON::Object::Ptr)) {
        auto versioned = core::dynamic::VersionedData_c_struct::formJSON(threadEntry.data);
...
std::vector<Thread> ThreadApiImpl::validateDecryptAndConvertThreadsDataToThreads(std::vector<server::ThreadInfo_c_struct> threads) {
    ...
    for (size_t i = 0; i < threads.size(); i++) {
        const auto& thread = threads[i];
        core::EncKeyLocation location{.contextId=thread.contextId, .resourceId=thread.resourceId};
        ...
        if(result[i].statusCode != 0) {
            ...
            try {
                ...
                auto tmp = decryptAndConvertThreadDataToThread(
                    thread,
                    thread.data.back(),
                    threadsKeys.at(location).at(thread.data.back().keyId)
                );
...
DecryptedMessageDataV4 ThreadApiImpl::decryptMessageDataV4(server::Message_c_struct message, const core::DecryptedEncKey& encKey) {
    try {
        auto encryptedMessageData = server::EncryptedMessageDataV4_c_struct::formJSON(message.data);
...
DecryptedMessageDataV5 ThreadApiImpl::decryptMessageDataV5(server::Message_c_struct message, const core::DecryptedEncKey& encKey) {
    try {
        auto encryptedMessageData = server::EncryptedMessageDataV5_c_struct::formJSON(message.data);
...
Message ThreadApiImpl::convertMessageDataV3ToMessage(server::Message_c_struct message, dynamic::MessageDataV3_c_struct messageData) {
    return convertServerMessageToLibMessage(
        ...
        core::Buffer::from(messageData.privateMeta),
        core::Buffer::from(messageData.data),
        std::string(),
        messageData.statusCode,
...
MessageDataSchema::Version ThreadApiImpl::getMessagesDataStructureVersion(server::Message_c_struct message) {
    ...
    if (message.data.type() == typeid(Poco::JSON::Object::Ptr)) {
        auto versioned = core::dynamic::VersionedData_c_struct::formJSON(message.data);
        switch (versioned.version) {
...
core::ModuleKeys ThreadApiImpl::threadToModuleKeys(server::ThreadInfo_c_struct thread) {
    return core::ModuleKeys{
        ...
        .moduleResourceId=thread.resourceId,
        .contextId = thread.contextId
    };
}
...
void ThreadApiImpl::assertThreadDataIntegrity(server::ThreadInfo_c_struct thread) {
    ...
    switch (getThreadEntryDataStructureVersion(thread_data_entry)) {
        ...
        case ThreadDataSchema::Version::VERSION_4:
            return;
        case ThreadDataSchema::Version::VERSION_5: {
            auto thread_data = core::dynamic::EncryptedModuleDataV5_c_struct::formJSON(thread_data_entry.data);
    ...
    try {
        switch (getMessagesDataStructureVersion(message)) {
            ...
            case MessageDataSchema::Version::VERSION_4:
                return 0;
            case MessageDataSchema::Version::VERSION_5: {
                auto encData = server::EncryptedMessageDataV5_c_struct::formJSON(message.data);
                ...
                if(
                    ...
                    !core::TimestampValidator::validate(dio.timestamp, (message.updates.empty() ? message.createDate : message.updates.back().createDate))
                ) {
