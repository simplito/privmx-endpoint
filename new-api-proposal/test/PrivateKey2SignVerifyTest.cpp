...
TEST(PrivateKey2Test, SignAndVerify) {
    ...
    PrivateKey2 priv1(raw_data);
    PublicKey2 pub1 = priv1.getPublicKey();
   
    // generate random pair keys
    PrivateKey2 priv2;
    PublicKey2 pub2 = priv2.getPublicKey();

    const string messageToSign("Sample message");
    Bytes data(Utils::s2b(messageToSign));

    Bytes sign1 = priv1.sign(data);
    EXPECT_TRUE(pub1.verify(data, sign1));

    Bytes sign2 = priv2.sign(data);
    EXPECT_TRUE(pub2.verify(data, sign2));

    EXPECT_FALSE(pub1.verify(data, sign2));
    EXPECT_FALSE(pub2.verify(data, sign1));
}
